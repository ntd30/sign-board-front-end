...
const ArticleCreate = (props) => {
    ...
    const onFinish = async (values) => {
        ...
        const { title, content, type, images } = values;
        // Tạo slug từ title
        const slug = title
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, '-')
        ...
        console.log("images", images.fileList[0].originFileObj)

        const imageFile = images?.fileList[0].originFileObj; // Lấy file gốc từ fileList

        const resCreateArticle = await createArticleAPI(title, content, type, slug, imageFile);
